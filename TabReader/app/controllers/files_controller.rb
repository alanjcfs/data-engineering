class FilesController < ApplicationController
  ...
  def index
    ...
    @items_purchasers = ItemsPurchaser.all
  ...
  def normalize_to_database(data)
    ...
    item = merchant.items.where("items.description LIKE ?", "%#{data[1]}%").first_or_create(price: (data[2].to_f * 100).to_i, description: data[1])
